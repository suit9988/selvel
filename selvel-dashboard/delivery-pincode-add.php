<?php
	...
	$parentPageURL = 'delivery-pincode.php';
	$pageURL = 'delivery-pincode-add.php';
	...
	if(isset($_POST['register'])){
		if($csrf->check_valid('post')) {
			$result = $admin->addDeliveryPincode($_POST, $_FILES);
	...
	if(isset($_GET['edit'])){
		...
		$data = $admin->getUniqueDeliveryPincodeById($id);
	}

	if(isset($_POST['id']) && !empty($_POST['id'])) {
		if($csrf->check_valid('post')) {
			$id = trim($admin->escape_string($admin->strip_all($_POST['id'])));
			$result = $admin->updateDeliveryPincode($_POST, $_FILES);
