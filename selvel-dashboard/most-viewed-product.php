<?php
	...
	$admin->checkUserPermissions('most_viewed_product', $loggedInUserDetailsArr);

	$pageName = "Most Viewed Product";
	$pageURL = 'most-viewed-product.php';
	...
	$whereArray[] = "product_id IN (select id from ".PREFIX."product_master)";

	if(count($whereArray)>0) {
		$whereCond = " where ".implode(' AND ', $whereArray);
	}

	$sql = "select product_id, COUNT(product_id) as prodCount from ".PREFIX.$tableName." ".$whereCond." group by product_id order by prodCount DESC";
