<?php
	...
	if(isset($_POST['update'])) {
		if($csrf->check_valid('post')) {
			...
			$faq = $_POST['faq'];
			if(empty($faq) ){
				header("location:".$pageURL."?updatefail&msg=Please enter faq details");
				exit();
			}
			else {
				//update to database
				if(empty($id)){
					$sql = "INSERT INTO ".PREFIX."cms_master(faq) VALUES ('".$faq."')";
					//echo $sql; exit;
					$admin->query($sql);
					header("location:".$pageURL."?updatesuccess");
				}else{
					$sql = "UPDATE ".PREFIX."cms_master SET `faq`='".$faq."'";
					//echo $sql; exit;
					$admin->query($sql);
...
<body>
										...
										<div class="card">
											...
											<div class="card-body">
												...
												<div class="form-group row">
													<div class="col-sm-12">
														<label>FAQS</label>
														<textarea name="faq" rows="7" id="faq"> <?php  echo $data['faq']; ?></textarea>
													</div>
												</div>
											</div>
										</div>
	...
	<script type="text/javascript">
		var editor = CKEDITOR.replace( 'faq', {
