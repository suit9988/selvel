...
$pid=$_GET['id'];
	$sql = "select * from ".PREFIX.$tableName." where product_id='$pid' order by id DESC";
							...
							<tbody>
								...
								?>
										<tr>
											...
											<td><?php $p_id= $row['product_id'];
												$sql_cat = "select * from ".PREFIX.product_master." where id='$p_id'";
											$results_cat = $admin->query($sql_cat);
										$row_cat = $admin->fetch($results_cat);
												echo $cc_id=$row_cat['product_name'];
														?></td>
